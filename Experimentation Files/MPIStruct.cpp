...
N-body problem for N particles, using OpenMPI. The simulation environment
is physically bounded within a sphere
...
File variance: Uses an MPI_Type_struct for sending data.
...
int main(int argc, char *argv[]) {
    ...
    // Commit an MPI type struct for commuication.
    // Stores same execution related data as the body_data type.
    MPI_Datatype MPI_body_data;
	int lengths[6] = {1, 1, 1, 1, 1, 1};
	const MPI_Aint displacements[6] = {0, sizeof(double), 2 * sizeof(double), 3 * sizeof(double), 4 * sizeof(double), 5 * sizeof(double)};
    MPI_Datatype types[6] = {MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE};

	MPI_Type_create_struct(6, lengths, displacements, types, &MPI_body_data);
    ...
    // *bodies = new body_data[(int)total_bodies];
    ...
    for (int count = 0; count < simulation_step; count++) {
        ...
        for (i = 0; i < processor_bodies; i++) {
                for (j = 0; j < (int)total_bodies; j++) {
                    ...
                    if (j != curr_body) {
                        ...
                        x_delta = proc_b[i].x_pos - bodies[j].x_pos;

                        // Displacement in y axis.
                        y_delta = proc_b[i].y_pos - bodies[j].y_pos;

                        // Displacement in z axis.
                        z_delta = proc_b[i].z_pos - bodies[j].z_pos;
        ...
        for (i = 0; i < processor_bodies; i++) {
            ...
            // Update position
            proc_b[i].x_pos += time_diff * proc_b[i].x_vel;
            proc_b[i].y_pos += time_diff * proc_b[i].y_vel;
            proc_b[i].z_pos += time_diff * proc_b[i].z_vel;

            // Update velocity
            proc_b[i].x_vel += gravity * forces[i][0];
            proc_b[i].y_vel += gravity * forces[i][1];
            proc_b[i].z_vel += gravity * forces[i][2];
            ...
            if (proc_b[i].x_pos < -OUT_OF_BOUNDS || proc_b[i].x_pos > OUT_OF_BOUNDS){
                proc_b[i].x_vel *= -1;
            }
            if (proc_b[i].y_pos < -OUT_OF_BOUNDS || proc_b[i].y_pos > OUT_OF_BOUNDS){
                proc_b[i].y_vel *= -1;
            }
            if (proc_b[i].z_pos < -OUT_OF_BOUNDS || proc_b[i].z_pos > OUT_OF_BOUNDS){
                proc_b[i].z_vel *= -1;
            }            
