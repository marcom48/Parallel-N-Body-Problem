...
File variance: Collapses nested loops using OpenMP's collapse clause.
...
int main(int argc, char *argv[]) {
    ...
    if (rank == root){
        ...
        // std::cin >> total_bodies;
        // std:: cin >> simulation_step;
        // std::cin >> time_diff;
        ...
        start_data[3] = num_threads;
        MPI_Bcast(start_data, 4, MPI_DOUBLE, 0, comm);
    ...
    // Heap allocating data to allow for scale to large test data.
    // I recognise this is slower than stack memory, but the main focus of
    ...
    for (int count = 0; count < simulation_step; count++) {
        ...
        // Parallelise with respect to each body but also each axis.
        // Collapse nested for loops.
        ...
        // Parallelise with respect to each body but also each axis.
        // Collapse nested for loops.
        #pragma omp parallel for schedule(static) private(i, curr_body, x_delta, y_delta, z_delta, euclidean_distance, gravity) collapse(2)
        for (i = 0; i < processor_bodies; i++) {
